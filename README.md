...
C) I don't use the USART Serial interrupt itself, because it could fire at any time and so causes problems with the LED control (explained above). Instead I look activly after every LED control into the USART Serial receive buffer for a new communication request from the app. The app only sends one char as a request, because this will arrive in the USART receive buffer just by hardware, independent of what the software is doing.  
