...
One important trick with state machines, respectively event driven transitions from state to state is to use a time tick if needed to repeat something or to move on. I use the same time tick for the periodically check of an app request, the handshake controlled communication and the progress in the LED programs. If there are needed more (like in my first attempt) they can simply be created in the ino sketch file within the model. More information about the implementation are within the documentation of the model.  
